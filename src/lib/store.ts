...
export interface Flashcard {
  ...
  isHard: boolean; 
  status?: "unknown" | "review" | "mastered"; // NEW VOCABULARY ENGINE STATE
  ...
  // Universal Knowledge Extraction Metadata
...
export const store = {
        ...
        decks.forEach(deck => {
            deck.cards.forEach(card => {
                ...
                if (savedState) {
                    ...
                    if (card.updatedAt && savedState.updatedAt) {
                        try {
                            const localTime = new Date(card.updatedAt).getTime();
                            const cloudTime = new Date(savedState.updatedAt).getTime();
                            if (localTime > cloudTime) {
                                localIsNewer = true;
                            }
                        } catch (e) {
                            console.warn("Timestamp parsing error during hydration", e);
                        }
                    }
                    ...
                    if (!localIsNewer) {
                        ...
                        card.status = typeof savedState.status === 'string' ? savedState.status : card.status;
  ...
  addDeck: async (deck: Deck) => {
    const parts = (deck.subject || "").split('/').map(p => p.trim()).filter(Boolean);
    if (parts.length > 3) {
      throw new Error("Danh mục không được vượt quá 4 cấp độ (Level 1 / Level 2 / Level 3).");
    }
  ...
  updateCardStatus: (deckId: string, cardId: string, status: "unknown" | "review" | "mastered") => {
     ...
     if (!deck) {
         ...
         if (!deck) deck = Object.values(tempDecks).find((d: any) => d.cards.some((c: any) => c.id === cardId));
     }
     if (!deck) return;
     ...
     card.status = status;
     if (status === "unknown") card.isHard = true;
     else card.isHard = false;
     ...
     if (currentUser && currentUser.id) {
         import('./offlineSync').then(({ OfflineSyncQueue }) => {
            OfflineSyncQueue.enqueueCardState(currentUser!.id, card.id, { status: card.status, isWeakCard: card.isHard, updatedAt: nowStr });
  ...
  updateCardMastery: (deckId: string, cardId: string, remembered: boolean) => {
     ...
     if (remembered) {
         ...
         card.isHard = false;
         if (card.status === "unknown" || card.status === "review") {
             card.status = undefined; // clear status once successfully reviewed
         }
         if (currentUser) {
             const isDoubleXP = currentUser.doubleXPUntil && currentUser.doubleXPUntil > Date.now();
             const isAchilles = currentUser.achillesUntil && currentUser.achillesUntil > Date.now();
         ...
         card.isHard = true;
         card.status = "unknown"; // X button means unknown
