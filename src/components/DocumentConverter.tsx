...
${textChunk}`,
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        const parsed = await PersistenceService.restoreDrafts();
        if (parsed) {
          if (parsed.activeImportTab) {
            setActiveImportTab((parsed.activeImportTab === "file" || parsed.activeImportTab === "text") && !isUserAdminOrTeacher ? "manual" : parsed.activeImportTab);
          }
          if (parsed.deckTitle !== undefined) setDeckTitle(parsed.deckTitle);
  ...
  const handleSaveDeck = useCallback(async () => {
    ...
    try {
      ...
      if (!validateHierarchyDepth(finalSubject)) {
        ...
        for (let s = 0; s < splitCount; s++) {
          ...
          subCards.forEach((c) => {
            ...
            const firestorePayload = {
              ...
              detectedDomain: c.detectedDomain || "",
              cardType: c.cardType || "",
              confidence: c.confidence || null,
              extractionReason: c.extractionReason || "",
              primaryPartOfSpeech: c.primaryPartOfSpeech || "",
              mastery: 0,
              nextReview: Date.now(),
              isHard: false,
