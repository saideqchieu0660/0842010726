...
import { buildHierarchy, HierarchicalNode, validateHierarchyDepth } from '../utils/hierarchy';

const DeckListContext = React.createContext<any>(null);

const getAllDecks = (node: HierarchicalNode): Deck[] => {
  let decks = [...node.decks];
  for (const child of Object.values(node.children)) {
    decks = [...decks, ...getAllDecks(child)];
  }
  return decks;
};

const HierarchicalCategoryRenderer = ({ node, isRoot = false }: { node: HierarchicalNode, isRoot?: boolean }) => {
  const [isExpanded, setIsExpanded] = useState(true);
  const {
    onCategoryQuiz, onCategoryReviewHardCards, onCategoryStudyAll,
    isAdmin, onEditDeck,
    isOnline, offlineDeckIds, downloadingDecks, handleDownloadOffline,
    togglePin, pinnedDecks, getCreatorLabel,
    editingCategory, newCategoryName, setNewCategoryName,
    isSavingCategoryName, handleRenameCategory, setEditingCategory
  ...
  const allDecksInTree = useMemo(() => getAllDecks(node), [node]);
  // Use node.name for display instead of full path
  ...
  return (
    <div className={cn("space-y-4 animate-in fade-in duration-300", !isRoot && "pl-4 sm:pl-8 border-l-2 border-zinc-200 dark:border-zinc-800")}>
      <div className={cn("flex items-center justify-between gap-4 pb-2 sticky top-0 z-30 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md pt-2 rounded-lg", isRoot ? "border-b border-orange-500/20 dark:border-zinc-800/60" : "")}>
        <div className="flex items-center gap-4">
          <button 
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center justify-center w-12 h-12 rounded-2xl bg-orange-500/10 text-orange-600 dark:text-orange-400 font-bold border border-orange-500/20 shadow-sm shrink-0 hover:bg-orange-500/20 transition-colors"
          >
            {hasChildren ? (
              <ChevronDown className={cn("w-6 h-6 transition-transform duration-300", !isExpanded && "-rotate-90")} />
            ) : (
          ...
          <div className="flex items-baseline gap-3">
            {editingCategory === subject ? (
              <div className="flex items-center gap-2">
                ...
                />
                <button onClick={() => handleRenameCategory(subject, newCategoryName)} disabled={isSavingCategoryName} className="p-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg shadow-sm"><Check className="w-5 h-5" /></button>
                <button onClick={() => setEditingCategory(null)} disabled={isSavingCategoryName} className="p-2 bg-zinc-300 dark:bg-zinc-700 hover:bg-zinc-400 dark:hover:bg-zinc-600 text-zinc-800 dark:text-zinc-200 rounded-lg shadow-sm"><X className="w-5 h-5" /></button>
              </div>
            ) : (
              <h4 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight uppercase flex items-center gap-2 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                {displayName}
                <div className="flex items-center ml-2" onClick={e => e.stopPropagation()}>
                  {isAdmin && subject !== "📌 ĐÃ GHIM" && (
                    <button onClick={() => { setEditingCategory(subject); setNewCategoryName(subject); }} className="text-zinc-400 hover:text-orange-500 p-1.5 rounded-lg hover:bg-orange-100 dark:hover:bg-zinc-800 transition" title="Đổi tên danh mục"><Edit3 className="w-5 h-5" /></button>
                  )}
                  {subject !== "📌 ĐÃ GHIM" && (
                    <button onClick={async () => {
                      if (!isOnline) { toast.error("Bạn đang offline, không thể tải xuống danh mục."); return; }
                      const { downloadCourseForOffline } = await import('../utils/offlineDb');
                      let successCount = 0;
                      const loadingToast = toast.loading(`Đang tải xuống ${allDecksInTree.length} học phần...`);
                      for (const deck of allDecksInTree) {
                        try { await downloadCourseForOffline(deck.id); successCount++; } catch (err) { console.error(`Failed to download ${deck.id}:`, err); }
                      }
                      toast.dismiss(loadingToast);
                      if (successCount > 0) { toast.success(`Đã tải xuống thành công ${successCount}/${allDecksInTree.length} học phần.`); window.dispatchEvent(new Event('henosis-offline-update')); }
                      else { toast.error("Tải xuống thất bại. Vui lòng thử lại."); }
                    }} className="text-zinc-400 hover:text-emerald-500 p-1.5 rounded-lg hover:bg-emerald-100 dark:hover:bg-zinc-800 transition" title="Tải toàn bộ bộ học trong phân mục này để học offline"><DownloadCloud className="w-5 h-5" /></button>
                  )}
                  {subject !== "📌 ĐÃ GHIM" && (
                    <button onClick={() => {
                      const categoryUrl = `${window.location.origin}/category/${encodeURIComponent(subject)}`;
                      const shareText = `📚 Danh mục: ${displayName}\n👉 Truy cập toàn bộ thẻ: ${categoryUrl}`;
                      navigator.clipboard.writeText(shareText).then(() => { toast.success("Đã sao chép link danh mục!"); }).catch((err) => { toast.error("Không thể sao chép, vui lòng thử lại."); });
                    }} className="text-zinc-400 hover:text-blue-500 p-1.5 rounded-lg hover:bg-blue-100 dark:hover:bg-zinc-800 transition" title="Chia sẻ toàn bộ đường link danh mục này"><Share2 className="w-5 h-5" /></button>
                  )}
                </div>
              </h4>
            )}
            <span className="text-sm sm:text-base font-black opacity-60 text-orange-600 dark:text-orange-400">
              ({allDecksInTree.length} bộ học)
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {onCategoryReviewHardCards && (() => {
            const hardCardsInCat = allDecksInTree.flatMap(d => d.cards || []).filter(c => c.status === "unknown" || c.isHard);
            return (
              <button onClick={() => hardCardsInCat.length > 0 && onCategoryReviewHardCards(subject, allDecksInTree)} disabled={hardCardsInCat.length === 0} className={cn("mr-1 text-xs font-black text-white px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 focus:outline-none shrink-0", hardCardsInCat.length > 0 ? "bg-gradient-to-r from-red-500 to-rose-500 hover:from-red-600 hover:to-rose-600 hover:scale-105 active:scale-95 shadow-[0_0_15px_rgba(239,68,68,0.4)] cursor-pointer" : "bg-red-300 dark:bg-red-900/50 opacity-50 cursor-not-allowed")} title={hardCardsInCat.length > 0 ? `Ôn lại ${hardCardsInCat.length} thẻ khó` : `Không có thẻ khó`}>
          ...
          {onCategoryStudyAll && (() => {
            const allCardsCount = allDecksInTree.reduce((acc, d) => acc + (d.cards?.length || 0), 0);
            if (allCardsCount > 0) {
              return (
                <button onClick={() => onCategoryStudyAll(subject, allDecksInTree)} className="mr-1 text-xs font-black bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white px-4 py-2.5 rounded-xl hover:scale-105 active:scale-95 transition-all shadow-md flex items-center gap-1.5 focus:outline-none cursor-pointer shrink-0" title="Học toàn bộ">
          ...
          {onCategoryQuiz && (
            <button onClick={() => onCategoryQuiz(subject, allDecksInTree)} className="mr-1 text-xs font-black bg-gradient-to-r from-orange-500 to-orange-500 hover:from-orange-600 hover:to-orange-600 text-black px-4 py-2.5 rounded-xl hover:scale-105 active:scale-95 transition-all shadow-md flex items-center gap-1.5 focus:outline-none cursor-pointer shrink-0" title="Thi AI">
          ...
          {hasDecks && node.decks.length > 1 && (
            <div className="flex items-center gap-2">
              <button onClick={() => scrollCategory('left')} className="w-10 h-10 rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 transition-all hover:scale-105 active:scale-95 focus:outline-none">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={() => scrollCategory('right')} className="w-10 h-10 rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 transition-all hover:scale-105 active:scale-95 focus:outline-none">
          ...
          >
            ...
            {hasChildren && (
              <div className="space-y-6 pt-2">
                {Object.values(node.children).sort((a,b) => a.name.localeCompare(b.name, 'vi', { numeric: true })).map(child => (
                  <HierarchicalCategoryRenderer key={child.id} node={child} isRoot={false} />
                ))}
              </div>
            )}

            {/* Render decks for this exact node */}
            {hasDecks && (
              <div
                    ...
                    <TiltCard key={`${deck.id || "deck"}-${idx}`} delayIdx={idx} className={cn("w-[85vw] sm:w-[380px] shrink-0 snap-start h-auto", isOfflineUnavailable && "opacity-40 grayscale pointer-events-none")}>
                      <div className="absolute top-4 right-4 z-20 flex gap-2">
                        ...
                        {isOnline && !offlineDeckIds.has(deck.id) && (
                           <button onClick={(e) => handleDownloadOffline(e, deck.id)} className="p-2 rounded-full transition-colors bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 border-2 border-transparent hover:border-emerald-500/30" title="Tải xuống để học Offline">
                        ...
                        {onEditDeck && (isAdmin || deck.createdBy === store.getCurrentUser()?.id) && (
                          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onEditDeck(deck); }} className="p-2 rounded-full transition-colors bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20" title="Sửa tên và danh mục">
                            <Edit3 className="w-5 h-5" />
                          </button>
                        )}
                        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); togglePin(deck.id); }} className={`p-2 rounded-full transition-colors ${pinnedDecks.includes(deck.id) ? 'bg-orange-500/20 text-orange-500 hover:bg-orange-500/30' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`} title={pinnedDecks.includes(deck.id) ? 'Bỏ ghim' : 'Ghim danh sách này'}>
...
const safeSetItem = (key: string, value: string) => {
  ...
  const handleRenameCategory = async (oldName: string, newName: string) => {
    ...
    if (!validateHierarchyDepth(trimmedNewName)) {
      toast.error("Danh mục không được vượt quá 4 cấp độ (Level 1 / Level 2 / Level 3).");
      return;
    }
  ...
  const hierarchyRoot = useMemo(() => {
    ...
    const unpinnedDecks = sortedAndFilteredDecks.filter(d => !pinnedDecks.includes(d.id));
    return buildHierarchy(unpinnedDecks);
  }, [sortedAndFilteredDecks, groupBySubject, pinnedDecks]);
          ...
          groupBySubject ? (
            <div className="space-y-16">
              {Object.entries(groupedDecks)
                .filter(([subject]) => subject === "📌 ĐÃ GHIM")
                ...
                <div key={subject} className="space-y-8 animate-in fade-in duration-300">
                  ...
                  <div className="flex items-center justify-between gap-4 border-b border-orange-500/20 dark:border-zinc-800/60 pb-4">
                    ...
                    <div className="flex items-center gap-3">
                      {onCategoryReviewHardCards && (() => {
                         const hardCardsInCat = subjectDecks.flatMap(d => d.cards || []).filter(c => c.status === "unknown" || c.isHard);
                         return (
                           <button
              ...
              {Object.values(hierarchyRoot)
                .sort((a,b) => a.name.localeCompare(b.name, 'vi', { numeric: true }))
                .map(node => (
                <HierarchicalCategoryRenderer key={node.id} node={node} isRoot={true} />
              ))}
            </div>
          ) : (
