...
export default function FlashcardViewer({ deckId, userId, deckCards }: FlashcardViewerProps) {
  ...
  // Auto Repair System
  useEffect(() => {
    if (currentCardIndex === -1 || !deckCards || deckCards.length === 0) return;
    const activeCard = deckCards[currentCardIndex];
    if (activeCard.metadataVersion !== 2 && activeCard.front) {
      ...
      import("../utils/apiClient").then(({ safeRequest }) => {
        safeRequest("/api/automation/repair-metadata", {
          ...
          body: JSON.stringify({
            ...
            wordForm: activeCard.wordForm
          })
        }).then(res => res.json()).then(data => {
          if (data.success && data.metadata && userId && deckId) {
            // Update in Firestore
            const cardRef = doc(db, `users/${userId}/decks/${deckId}/cards`, activeCard.id);
            // Notice: The actual card update logic might depend on how cards are stored.
            // If cards are stored in an array `cards` on the deck doc, we have to update the deck.
            // Let's check how they are stored. If they are subcollection, it's `cards/${activeCard.id}`.
          }
        }).catch(err => console.error("Auto-repair failed", err));
      });
    }
  }, [currentCardIndex, deckCards, userId, deckId]);
      ...
      >
        {isClozeMode && activeCard.example_sentence ? (
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              {/* Badges for V2 Vocabulary */}
              {!isFlipped && activeCard.metadataVersion === 2 && (
                <div className="flex flex-wrap justify-center gap-2 mb-2">
                  {activeCard.category && (
                    <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300">
                      {activeCard.category}
                    </span>
                  )}
                  {activeCard.cefrLevel && (
                    <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                      {activeCard.cefrLevel}
                    </span>
                  )}
                  {activeCard.frequency && (
                    <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300">
                      {activeCard.frequency}
                    </span>
                  )}
                  {activeCard.primaryPartOfSpeech && (
                    <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
                      {activeCard.primaryPartOfSpeech}
                    </span>
                  )}
                </div>
              ...
              {/* IPA */}
              {!isFlipped && activeCard.metadataVersion === 2 && activeCard.ipa && (
                <p className="font-mono text-lg text-zinc-500 dark:text-zinc-400">
                  {activeCard.ipa}
                </p>
              )}
