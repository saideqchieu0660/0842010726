...
import { MessageCircle, X, Send, Bot, CheckCircle, Maximize2, Minimize2, Flame, Plus, Loader2, FolderPlus, Settings, Sparkles, XCircle, Copy, Download, FileCode, Trash2 } from "lucide-react";
  ...
  const handleSaveToSetConfirm = async () => {
    ...
    try {
      ...
      if (isCreateNewSet) {
        ...
        const targetSubject = newSetSubject.trim() || "Tự chọn";
        if (!validateHierarchyDepth(targetSubject)) {
          setSaveErrorMsg("Danh mục không được vượt quá 4 cấp độ (Level 1 / Level 2 / Level 3).");
          setSavingCard(false);
          return;
        }
        ...
        const newDeckObj = {
          ...
          cards: [{ ...newCardObj, subject: targetSubject }]
        };
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          if (res.status === 429) {
            const errMsg = errData.message || (typeof errData.error === 'string' ? errData.error : "Bạn đang gọi AI quá nhanh. Hãy chờ hoặc nạp năng lượng!");
            setMessages(prev => [...prev, { role: "ai", text: `⏳ ${errMsg}` }]);
            if (errData.cooldownRemaining && typeof errData.cooldownRemaining === 'number') {
              const COOLDOWN_KEY = "ai_request_cooldown_timestamp";
              if (user) {
                localStorage.setItem(`${COOLDOWN_KEY}_${user.id}`, (Date.now() - (45000 - errData.cooldownRemaining * 1000)).toString());
                window.dispatchEvent(new CustomEvent("ai-cooldown-trigger", { detail: { userId: user.id } }));
              }
            }
            setIsLoading(false);
            fetchAgentLimits();
          ...
          throw new Error("API Agent 3 lỗi");
        }
      }
