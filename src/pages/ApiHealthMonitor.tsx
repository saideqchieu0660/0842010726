...
export function ServiceMonitor({
  ...
  const handleToggleChange = async (
    ...
    // --- CEREBRAS HANDSHAKE VALIDATION ---
    if (type === "groq" && newValue === true) {
      toast("Đang kiểm tra kết nối (Ping/Handshake) đến Cerebras Cluster...");
      try {
        const pingRes = await fetch("/api/admin/cerebras-ping", { method: "POST" });
        if (!pingRes.ok) {
          throw new Error("Handshake failed");
        }
        const pingData = await pingRes.json();
        if (!pingData.success) {
           throw new Error(pingData.error || "Unknown error");
        }
        toast.success("Kết nối Cerebras High-Speed Cluster thành công!");
      } catch (err: any) {
        toast.error("Cerebras Cluster Unreachable - Check Key. Khôi phục về OFF.");
          ...
          <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 p-6 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-1">
              ...
              >
                <div className="space-y-1 pr-4">
                  <div className="font-bold text-sm text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
                    <Zap className="w-4 h-4 text-orange-500" /> Cerebras High-Speed Cluster Control
                  </div>
                  <div className="text-[11px] text-zinc-500 flex items-center gap-1.5 mt-1">
                    {groqEnabled
                      ? <><span className="flex h-2 w-2 relative"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span><span className="font-mono text-green-700 dark:text-green-400 font-bold">ACTIVE / READY</span></>
