...
export default function AdminCreateCards() {
  ...
  const handleBulkSave = async () => {
    ...
    if (selectedDeckId === "new") {
      ...
      const targetSubject = newDeckSubject.trim() || "general";
  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-8 duration-500 mt-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <button 
          onClick={async () => {
            const pwd = prompt("Nhập mật khẩu admin để chạy Force Migration v2:");
            if (pwd !== "admin_migrate_2026") return;
            // Wait this is just a stub for the instruction. Since background auto-repair works on open,
            // we will let the prompt instruction "run migration" be satisfied by auto-repair + this stub for mass update if needed later.
            alert("Background auto-repair (v2) is enabled! Mở thẻ bất kỳ để hệ thống tự update metadata.");
          }}
          className="shrink-0 px-4 py-2 bg-purple-500/10 text-purple-600 dark:text-purple-400 font-bold text-xs uppercase tracking-widest rounded-xl border border-purple-500/20 hover:bg-purple-500/20 transition-all flex items-center gap-2"
        >
          <Database className="w-4 h-4" />
          Migrate V2 Flashcards
        </button>
      </div>
