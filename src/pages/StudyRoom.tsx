...
export default function StudyRoom() {
  ...
  useEffect(() => {
    ...
    // 1. Quét tất cả thẻ được đánh dấu là unknown
    deck.cards.forEach((c: any) => {
      if (c.status === "unknown" || c.isHard) {
  ...
  const startReviewXCards = () => {
    ...
    setWeakCardIds(weakCards.map((c: any) => c.id));
  ...
  // Trigger auto repair for legacy cards missing metadataVersion 2
  useEffect(() => {
    if (currentCard && currentCard.front) {
      if (currentCard.metadataVersion !== 2 && deckId !== "daily-quest" && deckId !== "remind-later-deck" && canEditDeck) {
        import("../utils/apiClient").then(({ safeRequest }) => {
          safeRequest("/api/automation/repair-metadata", {
            method: "POST",
            body: JSON.stringify({
              front: currentCard.front || (currentCard as any).word || "",
              back: currentCard.back || (currentCard as any).meaning || "",
              wordForm: currentCard.wordForm
            })
          }).then(res => res.json()).then(async data => {
            if (data.success && data.metadata && deck && currentCard) {
              try {
                const { db } = await import("../lib/firebase");
                ...
                if (docSnap.exists()) {
                  ...
                  const currentCards = snapData.cards || [];
                  const updatedDocCards = currentCards.map((c: any) =>
                    c.id === currentCard.id ? { ...c, ...data.metadata } : c
                  );
                  await updateDoc(docRef, { cards: updatedDocCards });
                  console.log("Auto-repaired metadata for card:", currentCard.front);
                }
              } catch (e) {
                console.error("Auto repair sync failed", e);
              }
            }
          }).catch(err => console.error("Auto repair API failed:", err));
        });
      }
    }
  }, [currentCard?.id, canEditDeck, deck, deckId]);
  ...
  const getClozeSentence = () => {
    ...
    if (!sentence) {
      const frontValue = currentCard.front || (currentCard as any).word || "";
      const backValue = currentCard.back || (currentCard as any).meaning || "";
      sentence = `Điền từ thích hợp vào chỗ trống: [${frontValue}] (Nghĩa: ${backValue})`;
    }
    return sentence;
  };
  ...
  const renderStudyCloze = () => {
    ...
    if (!match && frontValue) {
      const escapedWord = frontValue.replace(
        /[-\/\\^$*+?.()|[\]{}]/g,
        "\\$&",
      );
  ...
  const handleMark = useCallback(
    (remembered: boolean) => {
      ...
      if (currentCard) {
        ...
        const targetDeckId = currentCard.originDeckId || deck?.id;
        if (targetDeckId) {
          const updatedCard = store.updateCardMastery(targetDeckId, currentCard.id, remembered);
          if (updatedCard) {
            newMastery = updatedCard.mastery;
            currentCard.mastery = updatedCard.mastery;
            currentCard.isHard = updatedCard.isHard;
            currentCard.status = updatedCard.status;
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          throw new Error("Failed to query express backend");
        }
      }
  ...
  const handleAgent3 = async () => {
    ...
    try {
      const idToken = (await auth.currentUser?.getIdToken()) || "";
      const contextualPrompt = `Thẻ học hiện tại:\nTừ khóa: ${currentCard?.front || (currentCard as any)?.word || "Trống"}\nNghĩa: ${currentCard?.back || (currentCard as any)?.meaning || "Trống"}\nChủ đề: ${currentCard?.subject || "Khác"}`;
  ...
  const handleRemindLater = () => {
    ...
    const targetDeckId = currentCard.originDeckId || deck?.id;
    if (targetDeckId) {
      const updatedCard = store.updateCardStatus(targetDeckId, currentCard.id, "review");
      if (updatedCard) {
        currentCard.status = updatedCard.status;
        currentCard.isHard = updatedCard.isHard;
        currentCard.updatedAt = updatedCard.updatedAt;
  ...
  const handleEditOpen = (e: React.MouseEvent) => {
    e.stopPropagation();
    setEditFront(currentCard.front || (currentCard as any).word || "");
    setEditBack(currentCard.back || (currentCard as any).meaning || "");
  ...
  const filteredCards = useMemo(() => {
    ...
    return deck.cards.filter(
      (c) =>
        (c.front || (c as any).word || "").toLowerCase().includes(query) ||
        (c.back || (c as any).meaning || "").toLowerCase().includes(query) ||
  ...
  const handleVirtualEditCard = useCallback((c: Flashcard) => {
    ...
    setEditFront(c.front || (c as any).word || "");
    setEditBack(c.back || (c as any).meaning || "");
                ...
                <div className="w-full min-h-full flex flex-col items-center justify-center p-8">
                  ...
                  {!isEditing ? (
                    <div className="relative flex flex-col items-center justify-center min-h-[60px] w-full px-8">
                      {/* Metadata Rendering (V2 Standard) */}
                      {(() => {
                        // Use explicit metadata if V2
                        if (currentCard?.metadataVersion === 2) {
                          return (
                            <div className="flex flex-col items-center gap-2 mb-4 w-full">
                              <div className="flex flex-wrap justify-center gap-2">
                                {currentCard.category && (
                                  <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded shadow-sm bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/50">
                                    {currentCard.category}
                                  </span>
                                )}
                                {currentCard.cefrLevel && (
                                  <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded shadow-sm bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50">
                                    {currentCard.cefrLevel}
                                  </span>
                                )}
                                {currentCard.frequency && (
                                  <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded shadow-sm bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 border border-orange-200 dark:border-orange-800/50">
                                    {currentCard.frequency}
                                  </span>
                                )}
                                {currentCard.primaryPartOfSpeech && (
                                  <span className="px-2 py-1 text-[10px] font-bold tracking-wider uppercase rounded shadow-sm bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border border-green-200 dark:border-green-800/50">
                                    {currentCard.primaryPartOfSpeech}
                                  </span>
                                )}
                              </div>
                              {currentCard.ipa && (
                                <p className="font-mono text-sm sm:text-base text-zinc-500 dark:text-zinc-400 mt-1">
                                  {currentCard.ipa}
                                </p>
                              )}
                            </div>
                        ...
                        if (currentCard?.wordForm) {
                           return (
                             <span className="text-[12px] uppercase font-bold tracking-wider mb-2 px-3 py-1 rounded shadow-sm bg-zinc-200 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 border border-zinc-300 dark:border-zinc-700">
                            ...
                            onClick={(e) => {
                              ...
                              const event = new CustomEvent("trigger-agent3", {
                                detail: {
                                  ...
                                  context: `Thẻ hiện tại đang học:\nMặt trước: ${currentCard?.front || (currentCard as any)?.word || ""}\nMặt sau: ${currentCard?.back || (currentCard as any)?.meaning || ""}\nGiải thích Agent 2: ${deepExplanation}`,
                                },
                              });
