...
import { validateHierarchyDepth } from "../utils/hierarchy";
import {
  db,
  auth,
...
export default function StudentDashboard() {
  ...
  const handleSaveManualDeck = async () => {
    ...
    const targetSubject = manualSubject.trim() || "Tự chọn";
    if (!validateHierarchyDepth(targetSubject)) {
  ...
  const [showUnknownCardsModal, setShowUnknownCardsModal] = useState(false); // NEW STATE

  useEffect(() => {
    // AUTOMATIC VOCABULARY ENGINE STATE MIGRATION
    const runMigration = () => {
      try {
        const localReminders = JSON.parse(
        ...
        decks.forEach(deck => {
            ...
            deck.cards?.forEach(card => {
                if (localReminders.includes(card.id) && card.status !== "review") {
                    store.updateCardStatus(deck.id, card.id, "review");
                    deckChanged = true;
                } else if (card.isHard && card.status !== "unknown") {
                    store.updateCardStatus(deck.id, card.id, "unknown");
                    deckChanged = true;
                }
            });
            if (deckChanged) needsSync = true;
        });

        if (needsSync) {
            localStorage.removeItem("remind_later_items"); // clean up legacy list
        }
      } catch (e) {
  ...
  const allCardsNormalized = useMemo(() => {
  ...
  const unknownCards = useMemo(() => {
    return allCardsNormalized.filter((c) => c.status === "unknown" || (c.isHard && !c.status)); // fallback if migration hasn't hit yet
  }, [allCardsNormalized]);
  
  const remindLaterCards = useMemo(() => {
    return allCardsNormalized.filter((c) => c.status === "review");
  }, [allCardsNormalized]);

  const remindLaterCount = remindLaterCards.length;
  const unknownCardsCount = unknownCards.length;
  ...
  const startCategoryRemindLaterStudy = useCallback(
    (subject: string, subjectDecks: Deck[]) => {
      const reviewCards = subjectDecks
        .flatMap((d) =>
          (d.cards || []).map((c) => ({
            ...c,
            originDeckId: d.id,
      ...
      const remindDeck = {
        ...
        title: `Ôn tập thẻ nhắc nhở: ${subject}`,
        subject: subject,
        description: `Bộ thẻ gồm các từ vựng bạn cần ôn tập lại trong phân mục ${subject}.`,
        cards: reviewCards,
        createdAt: new Date().toISOString(),
        ownerId: "system",
      };
  ...
  const startCategoryUnknownStudy = useCallback(
    (subject: string, subjectDecks: Deck[]) => {
      const hardCards = subjectDecks
        ...
        .filter((c) => c.status === "unknown" || c.isHard);
      if (hardCards.length === 0) return;

      const remindDeck = {
        id: `unknown-cards-${subject.replace(/\s+/g, "-")}`,
        title: `Ôn tập thẻ khó: ${subject}`,
        subject: subject,
        description: `Bộ thẻ gồm các từ vựng khó (chưa thuộc) bạn đã đánh dấu trong phân mục ${subject}.`,
        cards: hardCards,
        createdAt: new Date().toISOString(),
        ownerId: "system",
      };
    ...
    if (categoryName && categoryDecks) {
      ...
      for (const deck of allDecks) {
        let cards = (deck.cards || []).filter((c) => {
          if (c.status === "unknown" || c.isHard) return true;
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          throw new Error("Lỗi kết nối từ Hệ thống Gemini");
        }
      }
      ...
      if (text) {
        let questions = parseRobustJsonArray(text);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex gap-2 shrink-0 w-full sm:w-auto items-center">
                          ...
                          <button
                            onClick={() => {
                              if (card.originDeckId) {
                                store.updateCardStatus(card.originDeckId, card.id, "mastered"); // Clearing status by setting to mastered or similar
                                // But store.updateCardStatus takes "unknown"|"review"|"mastered". "mastered" is fine.
                              }
                            }}
