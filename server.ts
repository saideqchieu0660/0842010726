...
// Setup HealthMonitor callbacks
HealthMonitor.firestoreSyncCallback = (index, metric) => {
   updateKeyMetrics(index, metric).catch(console.error);
};
HealthMonitor.firestoreLogCallback = (log, collection, docId) => {
   if (admin.apps.length > 0) {
      admin.firestore().collection(collection).doc(docId).set(log).catch(console.error);
   }
};
...
function getGroqKey(): { key: string; state: KeyState } {
  ...
  groqKeyStates.forEach(s => {
    ...
    if (s.status === "ISOLATED") {
        const unlockTime = (s as any).unlockTime || 0;
        if (now >= unlockTime) {
            s.status = "active";
            delete (s as any).unlockTime;
            addGroqRotationLog({
                toKeyIndex: s.index,
                reason: `Cerebras Key auto-recovered from 60s ISOLATED cooldown`
            });
        }
    } else if ((s.status === "rate_limited" || s.status === "failed") && s.lastUsed && (now - s.lastUsed.getTime() > 60000)) {
       const oldStatus = s.status;
       s.status = "active";
       addGroqRotationLog({
         toKeyIndex: s.index,
         reason: `Cerebras Key auto-recovered from ${oldStatus} cooldown (60s)`
       });
    }
  });
  ...
  while (attempts < groqKeyStates.length) {
    ...
    if (s.status === "HARD_LOCKED" || s.is_banned || s.status === "ISOLATED") {
...
function handleGroqError(state: KeyState, err: any) {
  ...
  } else if (errStatus === 429 || errStatus >= 500 || msg.includes("429") || msg.includes("50") || msg.includes("quota") || msg.toLowerCase().includes("too many requests") || msg.toLowerCase().includes("exhausted") || msg.toLowerCase().includes("limit exceed")) {
    state.status = "ISOLATED";
    const lockDuration = 60000; // 60 seconds
    const unlockTime = Date.now() + lockDuration;
    (state as any).unlockTime = unlockTime;
    
    addGroqRotationLog({
      toKeyIndex: state.index,
      reason: `Cerebras Key Isolated for 60s (Rate Limit/Server Error). Error: ${msg.substring(0, 100)}`
    });
    
    setTimeout(() => {
        if (state.status === "ISOLATED") {
            state.status = "active";
            delete (state as any).unlockTime;
            addGroqRotationLog({
                toKeyIndex: state.index,
                reason: `Cerebras Key Auto-Restored after 60s isolation.`
            });
        }
    }, lockDuration);
  } else {
...
async function executeGenerateContentRoundRobin(contents: any, config: any = {}): Promise<string> {
  ...
  // Completely isolated rotation pool for Cerebras Exclusive mode
  if (ENABLE_CEREBRAS() && groqKeyStates.length > 0) activeProviders.push("groq");

  if (activeProviders.length === 0) {
    throw new Error("Cerebras High-Speed Tier is currently disabled or has no active keys. Please enable it in the Admin Dashboard.");
  }
  ...
  for (let pIdx = 0; pIdx < numProviders; pIdx++) {
    ...
    // --- HARD THROTTLING DELAY (10-12s) - Only for non-Cerebras ---
    if (provider !== "groq") {
      const now = Date.now();
    ...
    try {
      ...
      if (provider === "groq") {
        ...
        for (let kIdx = 0; kIdx < numKeys; kIdx++) {
          ...
          // --- PER-KEY 10-12s THROTTLING DELAY ---
          const nowTime = Date.now();
          const throttleUntil = (state as any).throttleUntil || 0;
          if (nowTime < throttleUntil) {
             console.log(`[Key Throttle] Cerebras key ${state.index} is in 10-12s cooldown. Hopping to next key...`);
             continue; // hop to next key
          }
          const throttleDelay = Math.floor(Math.random() * 2000) + 10000;
          (state as any).throttleUntil = nowTime + throttleDelay;
...
const UNIVERSAL_EXTRACTION_ENGINE_RULES = `
========================================================
UNIVERSAL KNOWLEDGE EXTRACTION ENGINE (V3)
========================================================

STEP 1: DOMAIN IDENTIFICATION
Analyze the input to determine the knowledge domain(s). It may be English Vocabulary, Mathematics, Physics, Chemistry, Biology, History, Computer Science, etc.

STEP 2: EXTRACTION STRATEGY
Apply domain-specific extraction:
- English Vocabulary: Extract pronunciation, meaning, examples, collocations, etymology. Provide conceptual translation, not shallow dictionary translation.
- Mathematics: Extract definitions, formulas, symbols, theorems, proofs, common mistakes, worked examples.
- Physics: Extract concepts, physical quantities, formulas, units, laws, assumptions.
- Chemistry: Extract reactions, equations, structures, mechanisms, terminology.
- Biology: Extract concepts, processes, classifications, comparisons.
- Programming/CS: Extract concepts, syntax, algorithms, design patterns, best practices.
- History/General: Extract events, timelines, causes, comparisons, key ideas.

STEP 3: CARD TYPE SELECTION
Use the most appropriate card type, e.g.:
Vocabulary Card, Concept Card, Formula Card, Question Card, Definition Card, Comparison Card, Theorem Card, Algorithm Card, Case Study Card.

STEP 4: INTELLIGENT SPLITTING
Split large topics into atomic learning units. One flashcard = exactly one important idea. Do not merge unrelated concepts.

STEP 5: MISSING KNOWLEDGE DETECTION
Ask yourself: "What important information would a student likely miss if I only extracted vocabulary?"
Include formulas, assumptions, relationships, and key conclusions implicitly described in the text.
...
// JWT Helper for Firebase ID Tokens
  ...
  // New Autonomous Agent & AI Exam limits: Max 7 queries per day, shared between Agent 2, 3, and AI Exam Generator
  const AGENT_QUOTA_LIMIT = 7;
  const AGENT_COOLDOWN_DURATION = 45000; // 45 seconds

  const agentCooldowns = new Map<string, number>();
  const inMemoryAgentQuotas = new Map<string, { used: number; date: string }>();

  // Simple periodic cleanup to prevent memory growth
  setInterval(() => {
    const now = Date.now();
    for (const [key, timestamp] of agentCooldowns.entries()) {
      if (now >= timestamp) {
        agentCooldowns.delete(key);
      }
    }
  }, 60000);

  const getAgentUsageInfo = async (userId: string) => {
    const now = Date.now();
    const cooldownExpiry = agentCooldowns.get(userId) || 0;
    const cooldownRemaining = cooldownExpiry > now ? Math.ceil((cooldownExpiry - now) / 1000) : 0;

    let limitUsedToday = 0;
    ...
    if (userId && admin.apps.length > 0) {
      try {
        ...
        if (docSnap.exists) {
          const data = docSnap.data();
          const lastAgentUsedDate = data?.lastAgentUsedDate || todayStr;
          if (lastAgentUsedDate === todayStr) {
            limitUsedToday = typeof data?.agentLimitUsedToday === "number" ? data.agentLimitUsedToday : 0;
          }
        }
      } catch (err) {
        console.error("Error reading agent limit from Firestore:", err);
      }
    } else if (userId) {
      const inMemoryData = inMemoryAgentQuotas.get(userId);
      if (inMemoryData && inMemoryData.date === todayStr) {
        limitUsedToday = inMemoryData.used;
      }
    }

    const remaining = Math.max(0, AGENT_QUOTA_LIMIT - limitUsedToday);
    return {
      remaining,
      cooldownRemaining,
      limitUsedToday,
  ...
  const incrementAgentQuota = async (userId: string) => {
    ...
    if (userId && admin.apps.length > 0) {
      try {
        ...
        await db.runTransaction(async (transaction) => {
          ...
          if (doc.exists) {
            const data = doc.data();
            lastDate = data?.lastAgentUsedDate || todayStr;
            if (lastDate === todayStr) {
              used = typeof data?.agentLimitUsedToday === "number" ? data.agentLimitUsedToday : 0;
            }
          }
          currentUsed = used + 1;
          transaction.set(userRef, {
            agentLimitUsedToday: currentUsed,
            lastAgentUsedDate: todayStr
          }, { merge: true });
        });
        console.log(`[Agent Quota] Successfully incremented daily quota for user ${userId} to ${currentUsed}`);
      } catch (err) {
        console.error("Error incrementing agent limit in Firestore transaction:", err);
      }
    } else if (userId) {
      const inMemoryData = inMemoryAgentQuotas.get(userId) || { used: 0, date: todayStr };
      if (inMemoryData.date !== todayStr) {
        inMemoryData.used = 0;
        inMemoryData.date = todayStr;
      }
      inMemoryData.used += 1;
      inMemoryAgentQuotas.set(userId, inMemoryData);
    }
  };
  ...
  app.post("/api/convert-document", aiCooldownMiddleware, async (req, res, next) => {
    try {
      ...
      const processChunk = async (chunkWords: string[], i: number) => {
         const prompt = `[STRICT DETERMINISTIC MODE] You are a Data Compiler for the Henosis Flashcard System (V3 Standard).
Extract knowledge units from the raw text below and generate structured flashcards based on the domain.
...
[
  {
    "front": "Question, word, concept, or formula",
    "back": "Answer, meaning, definition, or explanation (SHORT and precise)",
    "ipa": "/string/",
    "primaryPartOfSpeech": "Noun|Verb|Adjective...",
    "category": "Vocabulary|Concept|Formula...",
    ...
    "confidence": 0.95,
    "extractionReason": "Short explanation of why this is important",
    "metadataVersion": 3
  }
]
- Return EXACTLY ONE JSON array, with no markdown code blocks.
- If no valid knowledge found, return [].
- Never infer POS dynamically on the frontend. Output it strictly here.
...
${reminderSuffix}`;
  ...
  res.json({
       totalKeys: HealthMonitor.getStates('google').length,
       currentIndex: 0,
       logs: HealthMonitor.googleLogs,
       keys: HealthMonitor.getStates('google').map(s => {
       ...
       openrouter: {
          totalKeys: HealthMonitor.getStates('openrouter').length,
          currentIndex: 0,
          logs: HealthMonitor.openRouterLogs,
          keys: HealthMonitor.getStates('openrouter').map(s => {
        ...
        groq: {
          totalKeys: HealthMonitor.getStates('cerebras').length,
          currentIndex: 0,
          logs: HealthMonitor.cerebrasLogs,
          keys: HealthMonitor.getStates('cerebras').map(s => {
   ...
   app.post("/api/admin/cerebras-ping", async (req, res, next) => {
  try {
    const envKeys = [
      process.env.VITE_CEREBRAS_KEY_1, process.env.VITE_CEREBRAS_KEY_2, process.env.VITE_CEREBRAS_KEY
    ].filter(k => k && k.trim() && k !== "undefined" && k !== "null").map(k => k!.trim());

    if (envKeys.length === 0) {
      return res.status(400).json({ error: "No Cerebras keys found in environment." });
    }

    const testKey = envKeys[0];
    const pingRes = await fetch("https://api.cerebras.ai/v1/models", {
      headers: { "Authorization": `Bearer ${testKey}` }
    });

    if (pingRes.ok) {
      return res.json({ success: true, message: "Handshake successful" });
    } else {
      return res.status(pingRes.status).json({ error: `Handshake failed: ${pingRes.status}` });
    }
  } catch (err: any) {
    return res.status(500).json({ error: `Handshake failed: ${err.message}` });
  }
});
...
app.post("/api/admin/api-toggles", express.json(), async (req, res) => {
   ...
   app.post("/api/admin/reset-keys-status", async (req, res, next) => {
     try {
       ...
       HealthMonitor.getStates('google').forEach(s => { s.status = "active"; s.errorCount = 0; });
       HealthMonitor.getStates('openrouter').forEach(s => { s.status = "active"; s.errorCount = 0; });
       HealthMonitor.getStates('cerebras').forEach(s => { s.status = "active"; s.errorCount = 0; });
       
       HealthMonitor.googleLogs.length = 0;
       HealthMonitor.openRouterLogs.length = 0;
       HealthMonitor.cerebrasLogs.length = 0;
       
       HealthMonitor.addLog('google', { id: `log-${Date.now()}-1`, timestamp: new Date().toISOString(), toKeyIndex: 1, reason: "[Admin] Khôi phục trạng thái toàn bộ Gemini API Keys về Active." });
       HealthMonitor.addLog('openrouter', { id: `log-${Date.now()}-2`, timestamp: new Date().toISOString(), toKeyIndex: 1, reason: "[Admin] Khôi phục trạng thái toàn bộ OpenRouter API Keys về Active." });
       HealthMonitor.addLog('cerebras', { id: `log-${Date.now()}-3`, timestamp: new Date().toISOString(), toKeyIndex: 1, reason: "[Admin] Khôi phục trạng thái toàn bộ Cerebras API Keys về Active." });
  ...
  app.post("/api/automation/process-chunk", async (req, res, next) => {
      ...
      try {
        ...
        const jsonNormalPrompt = `You are a Universal Knowledge Extraction Engine. Your goal is to identify and extract ALL prominent knowledge units (concepts, vocabulary, formulas, rules) from this source text into highly educational flashcards.
  ...
  "flashcards": [
    {
      ...
      "wordForm": "v., n., adj., adv., idiom, formula, concept",
      "back": "Answer, definition, or explanation (MUST be written in Vietnamese)",
      "example": "An illustrative sentence or mathematical example",
      "origin": "Etymology, mnemonic, or mathematical proof idea (optional)"
    }
  ]
}
\`\`\`
...
${textChunk}`;
  ...
  app.post("/api/admin/migrate-v2-flashcards", async (req, res, next) => {
    try {
      res.json({ message: "Migration should be triggered from the client using batch requests to /api/automation/repair-metadata" });
    } catch (e) {
  ...
  // Background hydration helper for incomplete/degraded flashcards
  app.post("/api/automation/repair-metadata", express.json(), async (req, res, next) => {
    try {
      const { front, back, wordForm } = req.body;
      if (!front) {
        return res.status(400).json({ error: true, message: "Missing front/word" });
      }

      const requestPrompt = `[HENOSIS Data Enrichment Policy & Metadata Repair]
You are a Knowledge Enrichment Engine. 
Your task is to repair and generate missing metadata for the flashcard (V3 Standard) while maximizing the preservation of user-provided information.

INPUT PRIORITY POLICY:
1. MAXIMIZE use of user-provided information. NEVER overwrite valid user definitions or notes.
2. If completeness >= 95%, ONLY normalize formatting.
3. If < 95%, generate ONLY missing metadata. Use internal knowledge first.
4. Normalize parts of speech to standard forms (Noun, Verb, Adjective, etc.).

Word/Concept: "${front}"
Meaning/Context (User Input): ${back || "unknown"}
Hinted POS/Domain (User Input): ${wordForm || "unknown"}
...
Do not include markdown or explanations.`;
      ...
      return res.json({
        ...
        metadata: {
          ipa: parsed.ipa,
          primaryPartOfSpeech: parsed.primaryPartOfSpeech,
          detectedDomain: parsed.detectedDomain,
          cardType: parsed.cardType,
          confidence: parsed.confidence,
          extractionReason: parsed.extractionReason,
  ...
  app.post("/api/automation/hydrate-card", async (req, res, next) => {
    try {
      ...
      const requestPrompt = `You are a Universal Knowledge Extraction Engine. Provide a high-quality illustrative example, sentence, or mathematical/scientific application for this knowledge unit.
          
Word/Concept: ${front}
Type/Domain: ${wordForm || "unknown"}
Meaning/Definition: ${back || "unknown"}

Return ONLY a minified JSON object with these EXACT keys:
{
  "example": "Illustrative example or application with its Vietnamese translation/explanation in parentheses immediately following.",
  "origin": "An appropriate context snippet matching the concept."
}
...
- Return EXACTLY ONE JSON object.`;
  ...
  app.post("/api/automation/validate-json", async (req, res, next) => {
    try {
      ...
      let usedKeyState: any = null;
      const requestPrompt = `[HENOSIS Data Enrichment Policy & JSON Validator]
You are a JSON Validator and Knowledge Enrichment Engine. 
Your task is to parse, repair, and enrich the following input into a STRICT JSON Array for Flashcards (V2 Standard).

INPUT PRIORITY POLICY:
1. MAXIMIZE use of user-provided information. NEVER overwrite valid user definitions, examples, or notes.
2. If completeness >= 95%, ONLY normalize formatting.
3. If < 95%, generate ONLY missing metadata. Use internal knowledge first. Do NOT generate missing definitions/examples unless they are entirely blank or corrupted.
4. Normalize parts of speech to standard forms (Noun, Verb, Adjective, etc.).
...
[
  {
    "front": "Question, word, concept, or formula",
    "back": "Answer, meaning, or explanation (Preserve user input if valid)",
    "ipa": "/string/",
    "primaryPartOfSpeech": "Noun|Verb|Adjective...",
    "detectedDomain": "English Vocabulary|Mathematics|Physics|Chemistry|Biology|Computer Science|General",
    "cardType": "Vocabulary Card|Concept Card|Formula Card|Question Card|Definition Card|Theorem Card",
...
- NO markdown \`\`\`json blocks.
- If input is entirely unparseable/empty, return [].
...
${jsonText}`;
  ...
  app.get("/api/automation/get-streaming-key", async (req, res) => {
    try {
      ...
      const activeKeys = states.filter(s => s.status === "active");
      if (activeKeys.length === 0) {
        if (states.length > 0) {
          return res.json({ success: true, key: states[0].key });
        }
  ...
  app.post("/api/admin/report-key-usage", express.json(), async (req, res) => {
    try {
      ...
      let fsIndex = 0;
      let internalProvider: 'google' | 'cerebras' | 'openrouter' = 'google';

      if (provider === "gemini") {
        internalProvider = 'google';
        matchedState = HealthMonitor.getStates('google').find(s => clean(s.key) === cleanReqKey);
        if (matchedState) fsIndex = matchedState.index;
      } else if (provider === "openRouter") {
        internalProvider = 'openrouter';
        matchedState = HealthMonitor.getStates('openrouter').find(s => clean(s.key) === cleanReqKey);
        if (matchedState) fsIndex = matchedState.index + 100;
      } else if (provider === "groq" || provider === "cerebras") {
        internalProvider = 'cerebras';
        matchedState = HealthMonitor.getStates('cerebras').find(s => clean(s.key) === cleanReqKey);
        if (matchedState) fsIndex = matchedState.index + 200;
      }
      ...
      if (metric === "usage") {
         HealthMonitor.reportUsage(internalProvider, matchedState.index);
      } else if (metric === "error") {
         HealthMonitor.reportError(internalProvider, matchedState.index, error?.includes('429'), false, `Client reported: ${error || "Unknown error"}`);
      }
