...
const regex = /  app\.post\("\/api\/automation\/manual-define", async \(req, res, next\) => \{[\s\S]*?    \} catch \(err: any\) \{[\s\S]*?      return res\.status\(500\)\.json\(\{ error: true, message: err\?\.message \|\| "Lỗi khi trích xuất định nghĩa\." \}\);\n    \}\n  \}\);/g;
